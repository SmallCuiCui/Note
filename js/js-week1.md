...
​	5种：number,boolean,string,undefined,null
​	特点：值在内存中占据固定大小空间，保存在栈内存中，复制时是新创建一个副本，不能给基本类型添加属性

##### 引用类型

​	2种：object(array),function
​	特点：由多个值构成保存在堆内存中，引用类型值的变量实际上包含的不是对象本身，而是指向对象的指针，复制时复制的是指针，因此两个变量指向同一对象，可以为其添加删除属性和方法

##### typeof()的返回

​	typeof()返回有六种值(变量类型)：number,boolean,string,undfined,object(数组也是对象),function。

​	值均为字符串，typeof(null)为object
...
​	-/*号：数字-字母数字，将运算符两边的数据隐式转换为数字类型，并进行减/乘/除运算，输出为数字
...
* if(){}else{}判断
  ...
  0、-0、null、""、false、undefined 或者 NaN在if条件里结果为false

* switch  case  多分支语句 应用于多个确定值需要判断的时候
...
* 采用形参接收
* arguments实参副本，可以接收函数传的所有参数，函数自带方法。
