...
块格式化上下文，决定了块级元素如何对它的内容进行布局，以及与其他元素的关系和相互关系。

BFC是一个独立的布局单元，它里面的元素与外面的元素不会发生影响。

#### 触发条件

* overflow： 不是visible
* display：inline-block，table-cell(表格单元格默认值)，table-caption(表格标题默认)，flow-root，flex
* float：不为none
* position：为absolute或fixed

#### 布局规则

* 浮动元素会被父级计算高度（父级触发BFC，解决高度塌陷问题）
* 浮动元素不会覆盖未浮动元素的位置（非浮动元素触发BFC）
* margin不会传递给父级（父级触发BFC）
* 相邻元素上下margin不会重叠（给其中一个元素增加父元素，并让父元素触发BFC）
